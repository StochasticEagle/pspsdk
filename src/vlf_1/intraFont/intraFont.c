...
/* Clut replaced by the ones used by vsh */

static u16 __attribute__((aligned(16))) clut[16] =
{
	0x0FFF, 0x1FFF, 0x2FFF, 0x3FFF, 0x4FFF, 0x5FFF, 0x6FFF, 0x7FFF, 
	0x8FFF, 0x9FFF, 0xAFFF, 0xBFFF, 0xCFFF, 0xDFFF, 0xEFFF, 0xFFFF
};

static u16 __attribute__((aligned(16))) clut_shadow[16] =
{
	0xF000, 0xF111, 0xF222, 0xF333, 0xF444, 0xF555, 0xF666, 0xF777, 
...
unsigned long* intraFontGetTable(SceUID fd, unsigned long n_elements, unsigned long bp_element) {
	...
	if (sceIoRead(fd, raw_table, len_table*sizeof(unsigned char)) != (len_table*sizeof(unsigned char))) {
		free(raw_table);
		return NULL;
	}
...
intraFont* intraFontLoad(const char *filename, unsigned short options) {
	...
	SceUID fd = sceIoOpen(filename, PSP_O_RDONLY, 0); /* read from the file in binary mode */
	if (fd < 0) {
		return NULL;
	}	

	unsigned long filesize = sceIoLseek32(fd, 0, PSP_SEEK_END);
    sceIoLseek32(fd, 0, PSP_SEEK_SET);
	
	//read pgf header
	PGF_Header header;
	if (sceIoRead(fd, &header, sizeof(PGF_Header)) != sizeof(PGF_Header)) {
		sceIoClose(fd);
		return NULL;
	}
	...
	//read advance table
    sceIoLseek32(fd, header.header_len+(header.table1_len+header.table2_len+header.table3_len)*8, PSP_SEEK_SET);
	...
	if (sceIoRead(fd, advancemap, header.advance_len*sizeof(signed long)*2) != (header.advance_len*sizeof(signed long)*2)) {
		free(advancemap);
		sceIoClose(fd);
		intraFontUnload(font);
		return NULL;
	...
	//read shadowmap
	unsigned long *ucs_shadowmap = intraFontGetTable(fd, header.shadowmap_len, header.shadowmap_bpe);
	...
	if (header.revision == 3) {
		if (sceIoRead(fd, font->charmap_compr, font->charmap_compr_len*sizeof(unsigned short)*2) != (font->charmap_compr_len*sizeof(unsigned short)*2)) {
			free(advancemap);		
		    free(ucs_shadowmap);
			sceIoClose(fd);
			intraFontUnload(font);
	...
	if (header.charmap_bpe == 16) { //read directly from file...
		if (sceIoRead(fd, font->charmap, header.charmap_len*sizeof(unsigned short)) != (header.charmap_len*sizeof(unsigned short))) {
			free(advancemap);	
			free(ucs_shadowmap);
			sceIoClose(fd);
			intraFontUnload(font);
		...
		unsigned long *id_charmap = intraFontGetTable(fd, header.charmap_len, header.charmap_bpe);
		if (id_charmap == NULL) {
			free(advancemap);	
			free(ucs_shadowmap);
			sceIoClose(fd);
	...
	//read charptr
	unsigned long *charptr = intraFontGetTable(fd, header.charptr_len, header.charptr_bpe);
	if (charptr == NULL) {
		free(advancemap);	
	    free(ucs_shadowmap);
		sceIoClose(fd);
	...
	//read raw fontdata
	unsigned long start_fontdata = sceIoLseek(fd, 0, PSP_SEEK_CUR);
	...
	if (sceIoRead(fd, font->fontdata, len_fontdata*sizeof(unsigned char)) != (len_fontdata*sizeof(unsigned char))) {
		free(advancemap);	
		free(ucs_shadowmap);
		free(charptr);
		sceIoClose(fd);
...
void intraFontActivate(intraFont *font, int shadow) {
	...
	sceGuClutLoad(1, (shadow) ? clut_shadow : clut);
...
float intraFontPrintfShadow(intraFont *font, float x, float y, const char *text, ...) {
	...
	return intraFontPrintShadow(font, x, y, buffer);
}
...
float intraFontPrintShadow(intraFont *font, float x, float y, const char *text) {
    ...
    x = intraFontPrintUCS2Shadow(font, x, y, ucs2_text);
...
float intraFontPrintUCS2(intraFont *font, float x, float y, const unsigned short *text) {
	...
	v = sceGuGetMemory(((n_glyphs)<<1) * sizeof(fontVertex));

	int c_index = n_sglyphs; // index for shadow and character/overlay glyphs	
	for(i = 0; i < length; i++) {
		...
		if (text[i] == '\n') {
			...
			width = 0.0f;
			height += font->advancey * glyphscale * 0.35f;			
		}
	}
		
	//finalize and activate texture (if not already active or has been changed)
	sceKernelDcacheWritebackAll();
	if (!(font->options & INTRAFONT_ACTIVE)) intraFontActivate(font, 0);

	sceGuBlendFunc(GU_ADD, GU_SRC_ALPHA, GU_ONE_MINUS_SRC_ALPHA, 0, 0);
	
	sceGuDisable(GU_DEPTH_TEST);
	sceGuDrawArray(GU_SPRITES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_2D, (n_glyphs)<<1, 0, v);
...
float intraFontPrintUCS2Shadow(intraFont *font, float x, float y, const unsigned short *text) {
	...
	//count number of glyphs to draw and cache BMPs
	int i, n_glyphs, last_n_glyphs, n_sglyphs, changed, count = 0;
	...
	//reserve memory in displaylist
	v = sceGuGetMemory(((n_sglyphs)<<1) * sizeof(fontVertex));
	...
	//finalize and activate texture (if not already active or has been changed)
	sceKernelDcacheWritebackAll();
	if (!(font->options & INTRAFONT_ACTIVE)) intraFontActivate(font, 1);

	sceGuBlendFunc(GU_REVERSE_SUBTRACT, GU_ONE_MINUS_DST_COLOR, GU_FIX, 0, 0xFFFFFFFF);
	
	sceGuDisable(GU_DEPTH_TEST);
	sceGuDrawArray(GU_SPRITES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_2D, (n_sglyphs)<<1, 0, v);
	sceGuEnable(GU_DEPTH_TEST);
